...
var apikey = 'd24c69c995862b081637c132308ec132';
...
function displayData(forecast){
	...
	$('.today-high').html(Math.round(forecast.daily.data[0].temperatureHigh));
	$('.today-icon').append('<img src="'+displayIcon(forecast.daily.data[0].icon)+'">');
	$('.today-low').html(Math.round(forecast.daily.data[0].temperatureLow));


	$('.day-1').html(displayDay(1));
	$('.high-1').html(Math.round(forecast.daily.data[1].temperatureHigh));
	$('.icon-1').append('<img src="'+displayIcon(forecast.daily.data[1].icon)+'">');
	$('.low-1').html(Math.round(forecast.daily.data[1].temperatureLow));

	$('.day-2').html(displayDay(2));
	$('.high-2').html(Math.round(forecast.daily.data[2].temperatureHigh));
	$('.icon-2').append('<img src="'+displayIcon(forecast.daily.data[2].icon)+'">');
	$('.low-2').html(Math.round(forecast.daily.data[2].temperatureLow));


	$('.day-3').html(displayDay(3));
	$('.high-3').html(Math.round(forecast.daily.data[3].temperatureHigh));
	$('.icon-3').append('<img src="'+displayIcon(forecast.daily.data[3].icon)+'">');
	$('.low-3').html(Math.round(forecast.daily.data[3].temperatureLow));


	$('.day-4').html(displayDay(4));
	$('.high-4').html(Math.round(forecast.daily.data[4].temperatureHigh));
	$('.icon-4').append('<img src="'+displayIcon(forecast.daily.data[4].icon)+'">');
	$('.low-4').html(Math.round(forecast.daily.data[4].temperatureLow));


	$('.day-5').html(displayDay(5));
	$('.high-5').html(Math.round(forecast.daily.data[5].temperatureHigh));
	$('.icon-5').append('<img src="'+displayIcon(forecast.daily.data[5].icon)+'">');
	$('.low-5').html(Math.round(forecast.daily.data[5].temperatureLow));


	$('.day-6').html(displayDay(6));
	$('.high-6').html(Math.round(forecast.daily.data[6].temperatureHigh));
	$('.icon-6').append('<img src="'+displayIcon(forecast.daily.data[6].icon)+'">');
	$('.low-6').html(Math.round(forecast.daily.data[6].temperatureLow));

	var arr = ["stay in bed. it's monday.","it's monday. do you really care?","f monday.","dear monday, f you.","i hate mondays.","if monday had a face, i would punch it.","monday should be optional.","today is monday and that's reason enough for me to hate it.","if each day is a gift, i would like to know where i can return mondays."];

function getRandom(){
  // get random number based on length of array
  var rand = Math.floor(Math.random() * arr.length);
  // display random value
  $('.mondaytext').html( arr[rand] );
}
...
getRandom();
		...
		if(displayDay(1) == "Mon"){
			$(".high-1").hide();
			$(".icon-1").replaceWith("<div class='icon-1'><img src='img/fck.svg'></div>");
			$(".low-1").hide();
		...
		if(displayDay(2) == "Mon"){
			$(".high-2").hide();
			$(".icon-2").replaceWith("<div class='icon-2'><img src='img/fck.svg'></div>");
			$(".low-2").hide();
		...
		if(displayDay(3) == "Mon"){
			$(".high-3").hide();
			$(".icon-3").replaceWith("<div class='icon-3'><img src='img/fck.svg'></div>");
			$(".low-3").hide();
		...
		if(displayDay(4) == "Mon"){
			$(".high-4").hide();
			$(".icon-4").replaceWith("<div class='icon-4'><img src='img/fck.svg'></div>");
			$("low-4").hide();
		...
		if(displayDay(5) == "Mon"){
			$(".high-5").hide();
			$(".icon-5").replaceWith("<div class='icon-5'><img src='img/fck.svg'></div>");
			$(".low-5").hide();
		...
		if(displayDay(6) == "Mon"){
			$(".high-6").hide();
			$(".icon-6").replaceWith("<div class='icon-6'><img src='img/fck.svg'></div>");
			$(".low-6").hide();
